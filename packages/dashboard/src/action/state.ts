...
import type { DebugActionProps } from "@phragon-util/action-service";
...
function reboot<State extends {} = DashboardStoreState>(
	...
	function done(data: ResData) {
		...
		if (data.ok) {
			...
			app.reload(state, props.init !== false);
		} else {
			const text = data.message || "Unknown error";
			const fail: { id: string; level: "error" } = {
				id: "api-state-error",
				level: "error",
			};
			if (dashboard.hasAction("toast")) {
				dashboard.emit("toast", {
					text,
					...fail,
				});
			} else {
				dashboard.emit<DebugActionProps>("debug", {
					message: text,
					...fail,
				});
			}
